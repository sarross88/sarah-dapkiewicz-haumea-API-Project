...
async function getBanner(imageNum, id) {
  try {
    const response = await fetch(
      `https://api.artic.edu/api/v1/artworks/${imageNum}`
    );
...
async function getImages(idImage) {
  try {
    const response = await fetch(
      `https://api.artic.edu/api/v1/artworks/${idImage}`
    );
    const data = await response.json();
    if (!response.ok) {
      throw new Error(response.status);
    ...
    function extractLastName(fullName) {
      if (fullName === null) {
        ...
        const name = fullName.toString().split(" ");
        let artistLastNameOnly = name[name.length - 1];
